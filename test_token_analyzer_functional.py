...
class FunctionalTokenizerTests(unittest.TestCase):
    ...
    def test_operator_longest_match(self):
        ops = ["<", "<-", "<="]
        t = FunctionalTokenizer(ops)
        src = "x <- y < z <= w"
        tokens = t.tokenize(src)
        values = [tok[1] for tok in tokens if tok[0] == "OP"]
    ...
    def test_definition_file_loading(self):
        with tempfile.TemporaryDirectory() as td:
            src_path = os.path.join(td, "sample.src")
            def_path = os.path.join(td, "sample.mydef")
    ...
    def test_keyword_prefix_error(self):
        t = FunctionalTokenizer(keywords=["for", "while"])
